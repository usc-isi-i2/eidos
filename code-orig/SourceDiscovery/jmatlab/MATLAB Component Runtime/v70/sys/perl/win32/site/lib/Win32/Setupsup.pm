...
# but it should work on win95 too (remove the app. line below)
#
...
{
	die "Usage: NT::Setupsup::DisableKeyboardAfterReboot()\n" if($#_ != -1);
	...
	if(!Win32::Registry::RegCreateKeyEx(&HKEY_LOCAL_MACHINE, 'SYSTEM\\CurrentControlSet\\Hardware Profiles\\' .
						...
						NULL, '', &NULL, &Win32::Registry::KEY_WRITE, NULL, $hKey, $disp))
	{
		Win32::Setupsup::SetLastError(Win32::GetLastError());
		return 0;
	}

	if(!Win32::Registry::RegSetValueEx($hKey, 'CSConfigFlags', NULL, &Win32::Registry::REG_DWORD, 1))
	{
		Win32::Setupsup::SetLastError(Win32::GetLastError());
...
{
	die "Usage: NT::Setupsup::EnableKeyboardAfterReboot()\n" if($#_ != -1);
	...
	if(!Win32::Registry::RegSetValueEx($hKey, 'CSConfigFlags', NULL, &Win32::Registry::REG_DWORD, 0))
	{
		Win32::Setupsup::SetLastError(Win32::GetLastError());
		Win32::Registry::RegCloseKey($hKey);
		return 0;
...
{
	die "Usage: NT::Setupsup::DisableMouseAfterReboot()\n" if($#_ != -1);
...
{
	die "Usage: NT::Setupsup::EnableMouseAfterReboot()\n" if($#_ != -1);
...
sub GetProgramFilesDir
{
	...
	if(!Win32::Registry::RegOpenKeyEx(&HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows\CurrentVersion',
					NULL, &Win32::Registry::KEY_READ, $hKey))
	{
		Win32::Setupsup::SetLastError(Win32::GetLastError());
		return 0;
	}
	...
	if(!Win32::Registry::RegQueryValueEx($hKey, 'ProgramFilesDir', NULL, $type, $_[0]))
	{
		Win32::Setupsup::SetLastError(Win32::GetLastError());
		Win32::Registry::RegCloseKey($hKey);
		return 0;
...
sub GetCommonFilesDir
{
	...
	if(!Win32::Registry::RegQueryValueEx($hKey, 'CommonFilesDir', NULL, $type, $_[0]))
	{
		Win32::Setupsup::SetLastError(Win32::GetLastError());
		Win32::Registry::RegCloseKey($hKey);
		return 0;
