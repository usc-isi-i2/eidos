...
% 
% USAGE:
%     [statistic, pvalue, H]=kolmorgorov(x,size,dist,varargin)
% 
% INPUTS:
...
%  dist     -  A char string of the name of the CDF, i.e. 'normcdf' for the normal, also use 'unifcdf' if you want to 
%              only pass already probability integral transformed data to the funtion, and set the varargin at to 0,1.
%  varargin -  Arguements passed to the CDF, such as D.F. for a T-dist
% 
% OUTPUTS:
%  statistic - The KS Statistic, D=max(abs(u-i/n)) where u is the ith sorted CDF point
%  pval      - The asymptotic probability of signifigance
%  H         - 1 for reject the null that the distribution is correct, 0 otherwise
% 
% 
% COMMENTS:
%  Note:  If you want to pass in already probability inegral transformed data by the suspect distribtion
...
d=max(abs(cdfvals-S));
statistic=d;
crit=kscritical(n,size);
H=statistic>crit;

k = (n^(0.5) + 0.12 + 0.11/n^(0.5))*statistic ;
tol=1;
pvalue=2*exp(-2*k*k);
s=-1;
i=2;
while tol >1e-10
    old=pvalue;
    pvalue=pvalue+2*s/exp(2*k*k*i*i);
    i=i+1;
    s=s*(-1);
    tol=abs(pvalue-old);
end
