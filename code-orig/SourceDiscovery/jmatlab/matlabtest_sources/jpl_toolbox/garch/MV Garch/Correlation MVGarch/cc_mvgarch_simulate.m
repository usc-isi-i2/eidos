...
elseif length(archP)==1
    archP=ones(k,1)*archP;
end

if isempty(garchQ)
    garchQ=ones(k,1);
elseif length(garchQ)==1
    garchQ=ones(k,1)*garchQ;
end
...
H=zeros(t+m,k);
for i=1:k
    parameters=garchparameters(index:index+archP(i,1)+garchQ(i,1));
    index=index+1+archP(i,1)+garchQ(i,1);
    constp=parameters(1);
    archp=parameters(2:archP(i,1)+1);
    garchp=parameters(archP(i,1)+2:archP(i,1)+garchQ(i,1)+1);
    ...
    T=size(data,1);
    for j = (m + 1):t+m
        h(j) = [1 ; data(j-(1:archP(i,1))).^2;  h(j-(1:garchQ(i,1))) ]'*parameters;
