...
//migration
var _ck=document.cookie,_cl=_sl=_ax=_bx=_als="";_ax=_II(_ck,"*AL:")+4;if(_II(_ck,"CP=")>=0&&_ax>3){_bx=_II(_ck,";",_ax);
_cl=_IS(_ck,_ax,(_bx>=0)?_bx:_IL(_ck))}_ax=_II(_ck,"WSS_MIGRATION=")+14;if(_ax>13){_bx=_II(_ck,";",_ax);_sl=_IS(_ck,_ax,(_bx>=0)?_bx:_IL(_ck))}
var _al=_IP(_cl,":");for(var i=0;i<_IL(_al);i++){if(_II(_sl,_al[i])<0){_sl+=":"+_IS(_al[i],0,12)}}_als=_sl;_al=_IP(_acct,";");var _ml=0;
for(var i=0;i<_IL(_al);i++){_al[i]=_IS(_al[i],0,12);if(_II(_als,_al[i])<0){_als+=(_IL(_als))?":"+_al[i]:_al[i];_ml=1}}if(!_ml||_sl=="*")_gn=_mgn;
