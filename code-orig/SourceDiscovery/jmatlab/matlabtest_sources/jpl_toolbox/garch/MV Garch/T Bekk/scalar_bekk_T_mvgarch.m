...
% PURPOSE:
%      To Estimate a scalar BEKK multivariate GARCH model with T-dist errors.  ****SEE WARNING AT END OF HELP FILE****
% 
% USAGE:
%      [parameters, loglikelihood, Ht, likelihoods, stdresid, stderrors, A, B, scores]  = scalar_bekk_T_mvgarch(data,p,q,BEKKoptions);
% 
% INPUTS:
...
% 
% OUTPUTS:
%      parameters    - A (k*(k+1))/2+p+q+1 vector of estimated parameteters. 
%                         To recover C, use ivech(parmaeters(1:(k*(k+1))/2), the last parameter is nu
...
%      *  NUMBER OF  PARAMETERS NEEDING TO BE ESTIMATED IS (k*(k+1))/2+p+q+1.  FOR A 10 VARIATE 
...
options=optimset(options,'TolCon',1e-3,'Display','off','Diagnostics','off','LargeScale','on','LevenbergMarquardt','on');
for i=1:k
...
CChol=chol(StartC)';
warning off
startingparameters=[vech(CChol);alpha0;beta0;nu];
...
parameters=fminunc('scalar_bekk_T_est_likelihood',startingparameters,options,data,p,q,k,k2,t);
parameters(length(parameters))=2.1+parameters(length(parameters))^2;
[loglikelihood,likelihoods,Ht]=scalar_bekk_T_likelihood(parameters,data,p,q,k,k2,t);
...
if nargout>=6
    %Std Errors
    A=hessian('scalar_bekk_T_likelihood',parameters,data,p,q,k,k2,t);
    ...
    for i=1:length(parameters)
        ...
        [HOLDER, indivlike] = scalar_bekk_T_likelihood(hparameters,data,p,q,k,k2,t);
