...
% PURPOSE:
%      To Estimate a full BEKK multivariate GARCH model with T-dist errors.  ****SEE WARNING AT END OF HELP FILE****
% 
% USAGE:
...
%      *  NEEDING TO BE ESTIMATED IS (k*(k+1))/2+pk^2+qk^2+1.  FOR A 5 VARIATE (1,1) MODEL THIS 
...
scalaropt=optimset(scalaropt,'TolFun',1e-1,'Display','off','Diagnostics','off','DiffMaxChange',1e-2);
...
parameters=fminunc('full_bekk_T_est_likelihood',startingparameters,options,data,p,q,k,k2,t);
parameters(length(parameters))=2.1+parameters(length(parameters))^2;
[loglikelihood,likelihoods,Ht]=full_bekk_T_likelihood(parameters,data,p,q,k,k2,t);
...
%Std Errors
if nargout>=6
    A=hessian('full_bekk_T_likelihood',parameters,data,p,q,k,k2,t);
    ...
    for i=1:length(parameters)
        ...
        [HOLDER, indivlike] = full_bekk_T_likelihood(hparameters,data,p,q,k,k2,t);
