...
public class EvaluatorImpl2 extends Evaluator {
	...
	public EvaluatorImpl2(DBConnection db, String dbName, boolean debug, int count1, int count2) {
		super(db,dbName);
		this.debug = debug;
		sampleCount1 = count1;
		sampleCount2 = count2;
	...
	private double evaluate(Clause c) {
		...
		for (ArrayList<String> inputTuple: targetInput.tuples) {	
			...
			if (numTargetTuples == 0 && numCandidateTuples == 0){
				...
				double fitness = numCommonTuples /(numTargetTuples * positiveScaling + (numCandidateTuples - numCommonTuples) / negativeScaling);
				if (debug) {
					System.out.println("fitness = numCommonTuples /(numTargetTuples * positiveScaling + (numCandidateTuples - numCommonTuples) / negativeScaling)");
					System.out.println("        = "+numCommonTuples+" /("+numTargetTuples+" * "+positiveScaling+" + ("+numCandidateTuples+" - "+numCommonTuples+") / "+negativeScaling+")");
