...
/*
 * cmdatatagutils.js 
 * $Id: cmdatatagutils.js 52122 2007-04-23 13:50:46Z jhoskins $
 * $Revision: 52122 $
 *
 * Version 4.1.0
 *
 ...
 * Tag Audit (cmSetAudit & cmAudit.js) Author - Joel Hoskins	6-22-2007 COPYRIGHT 2007 COREMETRICS, INC.
 * The following functions aid in the creation of Coremetrics data tags.
 * Added additional parameters to capture order payment type and shipping method.
 * Added additional parameters in shop tags to capture Manufacturer and additional data in shopping cart.
 * Included first party sub-domain name.
 */
 
var cm_ClientID = "90172959";
var cm_TrackLink = "A";
...
function cmSetProduction(){
	cm_HOST="cm.newegg.com/eluminate?"; 
}

/*
 * Calling this function turns on Automated Tag Audit
 */
function cmSetAudit(){						
	cmSetAudit = true;
	
	if (cmSetProduction)
	{cmSetProduction = false; 
	}
}
 ...
 * ManufactureName:  "SX1" additional field added. as well as an additional parameter "sx2".
 * 
 */
function cmCreateShopAction5Tag(productID,productName,productQuantity,productPrice,categoryID,manufactureName,extraField) {
	var pattern = /[^0-9\.]/gi;
    productPrice = productPrice.toString().replace(pattern, "");
	cmAddShop(["pr",productID,"pm",productName,"qt",productQuantity,"bp",productPrice,"cg",categoryID,"at","5","tid","4","pc","N","sx1",manufactureName,"sx2",extraField]);
}
 ...
 * ManufactureName:  "SX1" additional field added. as well as an additional parameter "sx2".
 */
function cmCreateShopAction9Tag(productID,productName,productQuantity,productPrice,customerID,orderID,orderTotal,categoryID,manufactureName,extraField) {
	var pattern = /[^0-9\.]/gi;
    productPrice = productPrice.toString().replace(pattern, "");
	orderTotal = orderTotal.toString().replace(pattern, "");
	cmAddShop(["pr",productID,"pm",productName,"qt",productQuantity,"bp",productPrice,"cg",categoryID,"cd",customerID,"on",orderID,"tr",orderTotal,"at","9","tid","4","pc","N","sx1",manufactureName,"sx2",extraField]);
	cmCalcSKUString();
}
