...
public class USZip implements Wrapper {
	...
	private Table getInfoByState(List<String> inputTuple) {
	...
	private Table getInfoByZip(List<String> inputTuple) {
	    ...
	    try {
	       ...
	       while ((line = br.readLine()) != null) {
	    	   if (line.indexOf("<Table>") != -1) {
	    		   // create output tuple
	    		   List<String> outputTuple = new ArrayList<String>(columns.size());
		           // initialize values to null
	    		   for (int i=0; i<columns.size(); i++) {
