...
public class Clause {
	...
	private static final int PREFIX_SIZE = 20;
	// clause definition
				...
				if (t != null) {
					if (t.isConstant) {
						...
						if (termName.get(key) == null) {
							...
							String val;
							if (t.lit == 0) {
								String typename = head().types[t.pos].name;
								val = truncateName(typename, PREFIX_SIZE) + t.pos;
							}
							else {
								int index = 0;
								for (int k=1; k<t.lit; k++) {
								...
								String typename = preds.get(t.lit).types[t.pos].name;
								val = truncateName(typename, PREFIX_SIZE) + (index + t.pos);
							}
							termName.put( key , val );
						}
						termName.put( i+","+j , termName.get(key) );
	...
	private String truncateName(String typename, int length) {
		String val;
		if (typename.length() < length) {
			val = typename;
		} else {
			val = typename.substring(0,length);
		}
		return val;
	}
					...
					for (int k=0; k<bodyTermArray.length; k++) {
						...
						if (term_k != null) {
							if (term_k.isConstant) {
								...
								boolean firstAppearance = !foundDistVar[headPos];
								if (firstAppearance) {
