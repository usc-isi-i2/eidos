...
public class EnumerateSources extends Enumerator {
	...
	public EnumerateSources(Domain domain,
			  				 int maxClauseLength,
			  				 int maxPredRepetition,
			  				 int maxVarLevel,
			  				 boolean noVarRepetition,
			  				 String heuristic,
			  				 boolean debug) {
		super(domain, maxClauseLength, maxPredRepetition, maxVarLevel, noVarRepetition, heuristic, debug);
	}
	...
	private ArrayList<Clause> addLiteral(Clause node) {
		...
		if (node.preds.isEmpty()) {
			...
			for (Predicate pred: (new Heuristic(heuristic,domain,maxClauseLength,maxPredRepetition)).orderedPreds(node)) {
