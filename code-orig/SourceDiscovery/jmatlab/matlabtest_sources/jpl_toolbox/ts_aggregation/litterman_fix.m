function res=litterman_fix(Y,x,ta,s,type,mu)
% PURPOSE: Temporal disaggregation using the Litterman method
%          mu parameter is fixed (supplied by the user)
% ------------------------------------------------------------
% SYNTAX: res=litterman(Y,x,ta,s,type,mu);
% ------------------------------------------------------------
...
%           res.rho     = innovational parameter (fixed, not estimated)
%           res.aic     = Information criterion: AIC
%           res.bic     = Information criterion: BIC
%           res.val     = Objective function used by the estimation method (=constant)
...
%        mu: innovational parameter 0 < mu < 1
% ------------------------------------------------------------
% LIBRARY: aggreg
% ------------------------------------------------------------
% SEE ALSO: litterman, chowlin, fernandez, td_plot, td_print
% ------------------------------------------------------------
...
% -----------------------------------------------------------
% Final estimation with fixed mu
...
Wi=inv(W);
beta=(X'*Wi*X)\(X'*Wi*Y);        % beta ML estimator
U=Y-X*beta;                      % Low frequency residuals
scp=U'*Wi*U;                     % Weighted least squares
sigma_a=scp/(N-p);               % sigma_a ML estimator
