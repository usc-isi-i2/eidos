...
B<SOURCENAME>), the function assumes that you are trying to open a backup
...
The function will fail if the log file already exists.
...
The I<Clear()> function clears the EventLog represented by $handle.
If you provide a non-null B<FILENAME>, the EventLog will be backed
up into B<FILENAME> before the EventLog is cleared. The function will
...
The I<Report()> function generates an EventLog entry. The HASHREF
...
The B<EventCategory> field can have any value you want. It is specific to
