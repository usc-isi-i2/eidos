...
# is there a more reliable way than this??
# this leaves $inst_root as, e.g., 'd:\perl'.
# update: yes, there is.  use $Config{prefix}
    my $inst_archlib = $Config{installsitearch};
    my $inst_root = $Config{prefix};
#    $inst_root =~ s/\\bin\\*.*$//i;
    my $packlist = "$Config{installsitearch}/auto/$current_package{'NAME'}/.packlist";
    my $cwd = getcwd();
    chdir($install_dir);
	...
	my ($inst_lib, $inst_bin, $inst_script, $inst_man1dir, $inst_man3dir, $inst_htmldir, $inst_htmlhelpdir);
	# PPM upgrade has to be done differently; needs to go into 'privlib'
	if ($current_package{'NAME'} eq 'ppm' || $current_package{'NAME'} eq 'PPM') {
	    $packlist = "$Config{archlibexp}/auto/$current_package{'NAME'}/.packlist";
    ...
    HtmlHelp::MakePerlHtmlIndexCaller();
...
During an installation, the following actions are performed:
    ...
    - if an <INSTALL> script is not provided, the package is assumed to be a
      ready-to-install (binary) Perl module, and the following default actions
      are performed:
