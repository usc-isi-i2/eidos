...
if ar==0
    ...
    [regressand,regressors]=lagmatrix(data,ar,1);
end
...
%LB(ar+2:ar+ma+1)=-inf;
%LB(ar+ma+2)=0.01;
%LB(ar+ma+3:ar+ma+2+p)=-inf;
%LB(ar+ma+p+3:ar+ma+2+p+q)=-inf;
...
if nargout >=4
    stderrors=hessian('garchinmeanlikelihood',parameters,regressand, regressors, ar, ma, p, q, m ,T,stdEstimate);
